...
function LinkAutomation({ notify, onBack }: { notify: (message: string, tone?: NonNullable<Toast>["tone"]) => void; onBack: () => void }) {
  ...
  function getScheduleName(scheduleUuid?: string) {
    return scheduleUuid === "SCH-DIRECT-LINK-02" ? "定向链接创建 · 厨电类" : scheduleUuid === "SCH-DIRECT-LINK-03" ? "定向链接创建 · 直播专场" : "定向链接创建 · 家电类";
  }
...
function LinkPagination({ page, pageCount, total, onChange }: { page: number; pageCount: number; total: number; onChange: (page: number) => void }) {
  if (total <= 0) return null;
  return <div className="link-task-pagination"><span>显示 {(page - 1) * 5 + 1}–{Math.min(page * 5, total)} 条，共 {total} 条</span><div><button disabled={page === 1} onClick={() => onChange(page - 1)}>‹</button>{Array.from({ length: pageCount }, (_, index) => <button key={index + 1} className={page === index + 1 ? "active" : ""} onClick={() => onChange(index + 1)}>{index + 1}</button>)}<button disabled={page === pageCount} onClick={() => onChange(page + 1)}>›</button></div></div>;
}
...
function ApiRecords({ tasks, notify }: { tasks: Array<{ id: string; yidaoId: string; created: string; api: string; status: string }>; notify: (message: string, tone?: NonNullable<Toast>["tone"]) => void }) {
  ...
  const recordPageCount = Math.max(1, Math.ceil(tasks.length / 5));
  const visibleRecords = tasks.slice((Math.min(recordPage, recordPageCount) - 1) * 5, Math.min(recordPage, recordPageCount) * 5);
  useEffect(() => { setRecordPage(1); }, [tasks.length]);
